...
<script>
	export default {
		...
		data() {
			return {
				hide: this.hidden,
			    pageCloak:this.pageCloak	// 小程序端初始化时，组件闪烁
			};
		},
		watch: {
			hidden(nVal, oVal){
...
.modal-view{
	...
	.modal-content{display:flex;justify-content: center;align-items: center;flex-direction: column;  background: #fff;width: 80%;border-radius: 6px;overflow: hidden;position: relative;padding-bottom: 50px;-webkit-transform: scale3d(.465, .465, .465); transform: scale3d(.465, .465, .465);transition: all .2s;will-change: auto;}
	...
	.modal-col {-ms-flex-preferred-size: 0;flex-basis: 0;-webkit-box-flex: 1;-ms-flex-positive: 1;flex-grow: 1;max-width: 100%;position: relative;transition: all .1s; display:flex;justify-content: center;align-items: center;}
	...
	.modal-text-center{display:flex;justify-content: center;align-items: center;font-size: 32upx;color:#333333;text-align:center}
